...
function task_1(filename)
    ...
    %Estimate all six parameters of the corresponding Gaussian mixture of two normal distributions (i.e., pA, pB, �A, �B, ?A, and ?B) using three different methods:
    ...
    disp('Part 3:');
    disp('1:');
    disp(['pA: ' sprintf('%f', size(classA')/size(classification(:))) ', pB: ' sprintf('%f', size(classB')/size(classification(:))) ', �A: ' sprintf('%f', mean(classA)) ', �B: ' sprintf('%f', mean(classB)) ', sA: ' sprintf('%f', std(classA,1)) ', and sB: ' sprintf('%f', std(classB,1))]);
